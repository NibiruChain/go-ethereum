...
func (eth *Ethereum) hashState(ctx context.Context, block *types.Block, reexec uint64, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	...
	for current.NumberU64() < origin {
		...
		_, _, _, err := eth.blockchain.Processor().Process(current, statedb, vm.Config{}, firehose.NoOpContext)
		if err != nil {
...
func (eth *Ethereum) stateAtTransaction(ctx context.Context, block *types.Block, txIndex int, reexec uint64, firehoseContext *firehose.Context) (*types.Transaction, vm.BlockContext, *state.StateDB, tracers.StateReleaseFunc, error) {
	...
	if beaconRoot := block.BeaconRoot(); beaconRoot != nil {
		...
		vmenv := vm.NewEVM(context, vm.TxContext{}, statedb, eth.blockchain.Config(), vm.Config{}, firehoseContext)
		core.ProcessBeaconBlockRoot(*beaconRoot, vmenv, statedb, firehoseContext)
	}
