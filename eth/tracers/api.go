	...
	go func() {
		...
		for number = start.NumberU64(); number < end.NumberU64(); number++ {
			...
			if beaconRoot := next.BeaconRoot(); beaconRoot != nil {
				...
				vmenv := vm.NewEVM(context, vm.TxContext{}, statedb, api.backend.ChainConfig(), vm.Config{}, firehose.NoOpContext)
				core.ProcessBeaconBlockRoot(*beaconRoot, vmenv, statedb, firehose.NoOpContext)
			}
...
func (api *API) IntermediateRoots(ctx context.Context, hash common.Hash, config *TraceConfig) ([]common.Hash, error) {
	...
	if beaconRoot := block.BeaconRoot(); beaconRoot != nil {
		vmenv := vm.NewEVM(vmctx, vm.TxContext{}, statedb, chainConfig, vm.Config{}, firehose.NoOpContext)
		core.ProcessBeaconBlockRoot(*beaconRoot, vmenv, statedb, firehose.NoOpContext)
	}
...
func (api *API) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if beaconRoot := block.BeaconRoot(); beaconRoot != nil {
		vmenv := vm.NewEVM(blockCtx, vm.TxContext{}, statedb, api.backend.ChainConfig(), vm.Config{}, firehose.NoOpContext)
		core.ProcessBeaconBlockRoot(*beaconRoot, vmenv, statedb, firehose.NoOpContext)
	}
	for i, tx := range txs {
...
func (api *API) traceTx(ctx context.Context, tx *types.Transaction, message *core.Message, txctx *Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewEVM(vmctx, vm.TxContext{GasPrice: big.NewInt(0)}, statedb, api.backend.ChainConfig(), vm.Config{Tracer: tracer.Hooks, NoBaseFee: true}, firehose.NoOpContext)
	...
	_, err = core.ApplyTransactionWithEVM(message, api.backend.ChainConfig(), new(core.GasPool).AddGas(message.GasLimit), statedb, vmctx.BlockNumber, txctx.BlockHash, tx, &usedGas, vmenv, firehose.NoOpContext)
	if err != nil {
