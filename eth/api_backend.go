...
func (b *EthAPIBackend) GetEVM(ctx context.Context, msg *core.Message, state *state.StateDB, header *types.Header, vmConfig *vm.Config, blockCtx *vm.BlockContext, firehoseContext *firehose.Context) *vm.EVM {
	if vmConfig == nil {
	...
	return vm.NewEVM(context, txContext, state, b.eth.blockchain.Config(), *vmConfig, firehoseContext)
}
