...
func TestAccountRange(t *testing.T) {
	...
	for i := range addrs {
		...
		addrs[i] = addr
		sdb.SetBalance(addrs[i], uint256.NewInt(1), tracing.BalanceChangeUnspecified, firehose.NoOpContext, "test")
		if _, ok := m[addr]; ok {
			t.Fatalf("bad")
		} else {
			m[addr] = true
...
func TestStorageRangeAt(t *testing.T) {
	...
	for _, entry := range storage {
		sdb.SetState(addr, *entry.Key, entry.Value, firehose.NoOpContext)
	}
