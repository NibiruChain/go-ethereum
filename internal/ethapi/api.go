...
func doCall(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64, firehoseContext *firehose.Context) (*core.ExecutionResult, error) {
	...
	evm, vmError := b.GetEVM(ctx, msg, state, header, &vm.Config{NoBaseFee: true}, &blockCtx, firehoseContext)
...
func executeEstimate(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, gasCap uint64, gasLimit uint64) (bool, *core.ExecutionResult, error) {
	...
	result, err := doCall(ctx, b, args, state, header, nil, nil, 0, gasCap, firehose.NoOpContext)
	if err != nil {
...
func AccessList(ctx context.Context, b Backend, blockNrOrHash rpc.BlockNumberOrHash, args TransactionArgs) (acl types.AccessList, gasUsed uint64, vmErr error, err error) {
	...
	for {
		...
		vmenv, _ := b.GetEVM(ctx, msg, statedb, header, &config, nil, firehose.NoOpContext)
