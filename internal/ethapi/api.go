...
func (diff *StateOverride) Apply(statedb *state.StateDB) error {
	...
	for addr, account := range *diff {
		...
		if account.Nonce != nil {
			statedb.SetNonce(addr, uint64(*account.Nonce), firehose.NoOpContext)
		}
		...
		if account.Balance != nil {
			...
			statedb.SetBalance(addr, u256Balance, tracing.BalanceChangeUnspecified, firehose.NoOpContext, firehose.IgnoredBalanceChangeReason)
		}
		...
		if account.State != nil {
			statedb.SetStorage(addr, *account.State, firehose.NoOpContext)
		}
...
func doCall(ctx context.Context, b Backend, args TransactionArgs, state *state.StateDB, header *types.Header, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64, firehoseContext *firehose.Context) (*core.ExecutionResult, error) {
	...
	evm := b.GetEVM(ctx, msg, state, header, &vm.Config{NoBaseFee: true}, &blockCtx, firehose.NoOpContext)
...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, blockOverrides *BlockOverrides, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	...
	return doCall(ctx, b, args, state, header, overrides, blockOverrides, timeout, globalGasCap, firehose.NoOpContext)
}
...
func (s *BlockChainAPI) Execute(ctx context.Context, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, blockOverrides *BlockOverrides) (hexutil.Bytes, error) {
	...
	result, err := doCall(ctx, s.b, args, state, header, overrides, blockOverrides, 5*time.Second, s.b.RPCGasCap(), firehoseContext)
...
func AccessList(ctx context.Context, b Backend, blockNrOrHash rpc.BlockNumberOrHash, args TransactionArgs) (acl types.AccessList, gasUsed uint64, vmErr error, err error) {
	...
	for {
		...
		vmenv := b.GetEVM(ctx, msg, statedb, header, &config, nil, firehose.NoOpContext)
