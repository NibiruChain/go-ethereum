...
func (b testBackend) GetEVM(ctx context.Context, msg *core.Message, state *state.StateDB, header *types.Header, vmConfig *vm.Config, blockContext *vm.BlockContext, firehoseContext *firehose.Context) (*vm.EVM, func() error) {
	...
	return vm.NewEVM(context, txContext, state, b.chain.Config(), *vmConfig, firehose.NoOpContext), vmError
}
