...
func (pre *Prestate) Apply(vmConfig vm.Config, chainConfig *params.ChainConfig,
	...
	if beaconRoot := pre.Env.ParentBeaconBlockRoot; beaconRoot != nil {
		evm := vm.NewEVM(vmContext, vm.TxContext{}, statedb, chainConfig, vmConfig, firehose.NoOpContext)
		core.ProcessBeaconBlockRoot(*beaconRoot, evm, statedb, firehose.NoOpContext)
	}
	...
	if miningReward >= 0 {
		...
		for _, ommer := range pre.Env.Ommers {
			...
			statedb.AddBalance(ommer.Address, uint256.MustFromBig(reward), tracing.BalanceIncreaseRewardMineUncle, false, firehose.NoOpContext, "test")
		}
		statedb.AddBalance(pre.Env.Coinbase, uint256.MustFromBig(minerReward), tracing.BalanceIncreaseRewardMineBlock, false, firehose.NoOpContext, "test")
	}
	// Apply withdrawals
	for _, w := range pre.Env.Withdrawals {
		// Amount is in gwei, turn into wei
		amount := new(big.Int).Mul(new(big.Int).SetUint64(w.Amount), big.NewInt(params.GWei))
		statedb.AddBalance(w.Address, uint256.MustFromBig(amount), tracing.BalanceIncreaseWithdrawal, false, firehose.NoOpContext, "test")
	}
	// Commit block
...
func MakePreState(db ethdb.Database, accounts types.GenesisAlloc) *state.StateDB {
	...
	for addr, a := range accounts {
		...
		statedb.SetBalance(addr, uint256.MustFromBig(a.Balance), tracing.BalanceIncreaseGenesisBalance, firehose.NoOpContext, "test")
