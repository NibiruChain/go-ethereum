...
func (t *StateTest) RunNoVerify(subtest StateSubtest, vmconfig vm.Config, snapshotter bool, scheme string) (state StateTestState, root common.Hash, err error) {
	...
	evm := vm.NewEVM(context, txContext, state.StateDB, config, vmconfig, firehose.NoOpContext)
	...
	state.StateDB.AddBalance(block.Coinbase(), new(uint256.Int), false, firehose.NoOpContext, "test")
