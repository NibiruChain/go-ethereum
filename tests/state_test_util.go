...
func (t *StateTest) RunNoVerify(subtest StateSubtest, vmconfig vm.Config, snapshotter bool, scheme string) (st StateTestState, root common.Hash, err error) {
	...
	evm := vm.NewEVM(context, txContext, st.StateDB, config, vmconfig, firehose.NoOpContext)
	...
	st.StateDB.AddBalance(block.Coinbase(), new(uint256.Int), tracing.BalanceChangeUnspecified, false, firehose.NoOpContext, "test")
...
func MakePreState(db ethdb.Database, accounts types.GenesisAlloc, snapshotter bool, scheme string) StateTestState {
	...
	for addr, a := range accounts {
		...
		statedb.SetBalance(addr, uint256.MustFromBig(a.Balance), tracing.BalanceChangeUnspecified, firehose.NoOpContext, "test")
