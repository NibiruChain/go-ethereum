...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	if header.ParentBeaconRoot != nil {
		...
		vmenv := vm.NewEVM(context, vm.TxContext{}, env.state, w.chainConfig, vm.Config{}, firehose.NoOpContext)
		core.ProcessBeaconBlockRoot(*header.ParentBeaconRoot, vmenv, env.state, firehose.NoOpContext)
	}
	return env, nil
}
...
func (w *worker) generateWork(params *generateParams) *newPayloadResult {
	...
	block, err := w.engine.FinalizeAndAssemble(w.chain, work.header, work.state, work.txs, nil, work.receipts, params.withdrawals, firehose.NoOpContext)
	if err != nil {
		return &newPayloadResult{err: err}
	}
...
func (w *worker) commit(env *environment, interval func(), update bool, start time.Time) error {
	if w.isRunning() {
		...
		block, err := w.engine.FinalizeAndAssemble(w.chain, env.header, env.state, env.txs, nil, env.receipts, nil, firehose.NoOpContext)
		if err != nil {
			return err
		}
