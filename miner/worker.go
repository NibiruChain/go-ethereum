...
func (miner *Miner) generateWork(params *generateParams) *newPayloadResult {
	...
	block, err := miner.engine.FinalizeAndAssemble(miner.chain, work.header, work.state, &body, work.receipts, firehose.NoOpContext)
	if err != nil {
		return &newPayloadResult{err: err}
	}
...
func (miner *Miner) prepareWork(genParams *generateParams) (*environment, error) {
	...
	if header.ParentBeaconRoot != nil {
		...
		vmenv := vm.NewEVM(context, vm.TxContext{}, env.state, miner.chainConfig, vm.Config{}, firehose.NoOpContext)
...
func (miner *Miner) applyTransaction(env *environment, tx *types.Transaction) (*types.Receipt, error) {
	...
	receipt, err := core.ApplyTransaction(miner.chainConfig, miner.chain, &env.coinbase, env.gasPool, env.state, env.header, tx, &env.header.GasUsed, vm.Config{}, firehose.NoOpContext)
	if err != nil {
