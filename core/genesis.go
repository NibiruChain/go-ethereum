...
func hashAlloc(ga *types.GenesisAlloc, isVerkle bool) (common.Hash, error) {
	...
	// [Firehose] Since everything in there is ephemeral, there is no reason to output any of those
	for addr, account := range *ga {
		if account.Balance != nil {
			statedb.AddBalance(addr, uint256.MustFromBig(account.Balance), tracing.BalanceIncreaseGenesisBalance, false, firehose.NoOpContext, firehose.IgnoredBalanceChangeReason)
		}
