...
func TestOpenDrops(t *testing.T) {
	...
	statedb.AddBalance(crypto.PubkeyToAddress(gapper.PublicKey), big.NewInt(1000000), false, firehose.NoOpContext, "test")
	statedb.AddBalance(crypto.PubkeyToAddress(dangler.PublicKey), big.NewInt(1000000), false, firehose.NoOpContext, "test")
	statedb.AddBalance(crypto.PubkeyToAddress(filler.PublicKey), big.NewInt(1000000), false, firehose.NoOpContext, "test")
	statedb.SetNonce(crypto.PubkeyToAddress(filler.PublicKey), 3, firehose.NoOpContext)
	statedb.AddBalance(crypto.PubkeyToAddress(overlapper.PublicKey), big.NewInt(1000000), false, firehose.NoOpContext, "test")
	statedb.SetNonce(crypto.PubkeyToAddress(overlapper.PublicKey), 2, firehose.NoOpContext)
	statedb.AddBalance(crypto.PubkeyToAddress(underpayer.PublicKey), big.NewInt(1000000), false, firehose.NoOpContext, "test")
	statedb.AddBalance(crypto.PubkeyToAddress(outpricer.PublicKey), big.NewInt(1000000), false, firehose.NoOpContext, "test")
	statedb.AddBalance(crypto.PubkeyToAddress(exceeder.PublicKey), big.NewInt(1000000), false, firehose.NoOpContext, "test")
	statedb.AddBalance(crypto.PubkeyToAddress(overdrafter.PublicKey), big.NewInt(1000000), false, firehose.NoOpContext, "test")
	statedb.AddBalance(crypto.PubkeyToAddress(overcapper.PublicKey), big.NewInt(10000000), false, firehose.NoOpContext, "test")
...
func TestOpenIndex(t *testing.T) {
	...
	// Create a blob pool out of the pre-seeded data
	statedb, _ := state.New(types.EmptyRootHash, state.NewDatabase(rawdb.NewDatabase(memorydb.New())), nil)
	statedb.AddBalance(addr, big.NewInt(1_000_000_000), false, firehose.NoOpContext, "test")
	statedb.Commit(0, true)
...
func TestOpenHeap(t *testing.T) {
	...
	// Create a blob pool out of the pre-seeded data
	statedb, _ := state.New(types.EmptyRootHash, state.NewDatabase(rawdb.NewDatabase(memorydb.New())), nil)
	statedb.AddBalance(addr1, big.NewInt(1_000_000_000), false, firehose.NoOpContext, "test")
	statedb.AddBalance(addr2, big.NewInt(1_000_000_000), false, firehose.NoOpContext, "test")
	statedb.AddBalance(addr3, big.NewInt(1_000_000_000), false, firehose.NoOpContext, "test")
	statedb.Commit(0, true)
...
func TestAdd(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for acc, seed := range tt.seeds {
			...
			statedb.AddBalance(addrs[acc], new(big.Int).SetUint64(seed.balance), false, firehose.NoOpContext, "test")
