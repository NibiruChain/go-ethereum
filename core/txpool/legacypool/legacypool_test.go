...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(uint256.Int).SetUint64(params.Ether), tracing.BalanceChangeUnspecified, firehose.NoOpContext, "test")
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(uint256.Int).SetUint64(params.Ether), tracing.BalanceChangeUnspecified, firehose.NoOpContext, "test")
...
func testAddBalance(pool *LegacyPool, addr common.Address, amount *big.Int) {
	pool.mu.Lock()
	pool.currentState.AddBalance(addr, uint256.MustFromBig(amount), tracing.BalanceChangeUnspecified, false, firehose.NoOpContext, "test")
	pool.mu.Unlock()
}
...
func TestChainFork(t *testing.T) {
	...
	resetState := func() {
		statedb, _ := state.New(types.EmptyRootHash, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
		statedb.AddBalance(addr, uint256.NewInt(100000000000000), tracing.BalanceChangeUnspecified, false, firehose.NoOpContext, "test")
...
func BenchmarkMultiAccountBatchInsert(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		key, _ := crypto.GenerateKey()
		account := crypto.PubkeyToAddress(key.PublicKey)
		pool.currentState.AddBalance(account, uint256.NewInt(1000000), tracing.BalanceChangeUnspecified, false, firehose.NoOpContext, "test")
