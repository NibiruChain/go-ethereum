...
type (
	...
	TransferFunc func(StateDB, common.Address, common.Address, *uint256.Int, *firehose.Context)
...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	if evm.firehoseContext.Enabled() {
		...
		evm.firehoseContext.RecordCallParams("CALL", caller.Address(), addr, value.ToBig(), gas, input)
	}
	...
	if isPrecompile {
		ret, gas, err = RunPrecompiledContract(p, input, gas, evm.Config.Tracer, evm.firehoseContext)
	} else {
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	if evm.firehoseContext.Enabled() {
		...
		evm.firehoseContext.RecordCallParams("CALLCODE", caller.Address(), addr, value.ToBig(), gas, input)
	}
...
func (evm *EVM) DelegateCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	if evm.firehoseContext.Enabled() {
		...
		evm.firehoseContext.RecordCallParams("DELEGATE", parent.Address(), addr, parent.value.ToBig(), gas, input)
	}
...
func (evm *EVM) StaticCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	// future scenarios
	evm.StateDB.AddBalance(addr, new(uint256.Int), tracing.BalanceChangeTouchAccount, isPrecompile, evm.firehoseContext, firehose.IgnoredBalanceChangeReason)
	...
	if isPrecompile {
		...
		// Initialise a new contract and set the code that is to be used by the EVM.
		// The contract is a scoped environment for this execution context only.
		contract := NewContract(caller, AccountRef(addrCopy), new(uint256.Int), gas, evm.firehoseContext)
		contract.SetCallCode(&addrCopy, evm.StateDB.GetCodeHash(addrCopy), evm.StateDB.GetCode(addrCopy))
		// When an error was returned by the EVM or when setting the creation code
		// above we revert to the snapshot and consume any gas remaining. Additionally
		// when we're in Homestead this also counts for code storage gas errors.
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *uint256.Int, address common.Address, typ OpCode) (ret []byte, createAddress common.Address, leftOverGas uint64, err error) {
	if evm.firehoseContext.Enabled() {
		...
		evm.firehoseContext.RecordCallParams("CREATE", caller.Address(), address, value.ToBig(), gas, nil)
	}
	if evm.Config.Tracer != nil {
	...
	if err == nil {
		...
		if contract.UseGas(createDataGas, evm.Config.Tracer, tracing.GasChangeCallCodeStorage, firehose.GasChangeReason("code_storage")) {
	...
	if err != nil && (evm.chainRules.IsHomestead || err != ErrCodeStoreOutOfGas) {
		...
		if err != ErrExecutionReverted {
			contract.UseGas(contract.Gas, evm.Config.Tracer, tracing.GasChangeCallFailedExecution, firehose.FailedExecutionGasChangeReason)
		} else {
			if evm.firehoseContext.Enabled() {
				evm.firehoseContext.RecordCallReverted()
			}
