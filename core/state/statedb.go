...
func (s *StateDB) AddBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason, isPrecompiledAddr bool, firehoseContext *firehose.Context, firehoseReason firehose.BalanceChangeReason) {
	stateObject := s.getOrNewStateObject(addr, isPrecompiledAddr, firehoseContext)
	if stateObject != nil {
		stateObject.AddBalance(amount, reason, firehoseContext, firehoseReason)
	}
}

// SubBalance subtracts amount from the account associated with addr.
func (s *StateDB) SubBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason, firehoseContext *firehose.Context, firehoseReason firehose.BalanceChangeReason) {
	stateObject := s.getOrNewStateObject(addr, false, firehoseContext)
	if stateObject != nil {
		stateObject.SubBalance(amount, reason, firehoseContext, firehoseReason)
	}
}

func (s *StateDB) SetBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason, firehoseContext *firehose.Context, firehoseReason firehose.BalanceChangeReason) {
	stateObject := s.getOrNewStateObject(addr, false, firehoseContext)
	if stateObject != nil {
		stateObject.SetBalance(amount, reason, firehoseContext, firehoseReason)
	}
}
...
func (s *StateDB) SelfDestruct(addr common.Address, firehoseContext *firehose.Context) {
	stateObject := s.getStateObject(addr)
	if stateObject == nil {
		return
	}
...
func (s *StateDB) Selfdestruct6780(addr common.Address, firehoseContext *firehose.Context) {
	...
	if stateObject.created {
		s.SelfDestruct(addr, firehoseContext)
	} else {
		// Firehose must emit a `RecordSuicide` even if the account was not created within this
		// transaction. which is what `stateObject.created` actually means.
		//
		// Indeed, `RecordSuicide` creates the sub balance change for the account that self-destructs
		// and user's still need to know that the account suicided even if internally `geth` does nothing.
		//
